...
/*
* pixel color ceiling floor :
* Cette fonction est chargée de
* colorer les pixels
* du sol et du plafond. Elle prend
* en entrée un pointeur vers une 
* structure de jeu t_game et la
* coordonnée x actuelle.
* Elle commence par obtenir les
* couleurs du plafond et du sol
* game->f_color et game->c_color
* en utilisant get_color.
* Ensuite, avec la boucle,
* elle colore les pixels du bord
* supérieur de l'écran jusqu'au point 
* où les murs sont dessinés. 
* Dans cet intervalle,
* elle assigne la couleur des pixels dans
* le tampon de jeu (game->buf[x][y])
* à la couleur du plafond (game->c_color);
* Ensuite, elle utilise une
* autre boucle while pour colorer
* les pixels du point où se terminent
* les murs jusqu'au bas de l'écran.
*/
...
* Utilise une boucle et parcourt les lignes
* verticales de la fenêtre de visualisation,
* de la coordonnée y game->draw_start
* jusqu'à game->draw_end
* et calcule la coordonnée de la texture (game->tex_y)
* et la couleur correspondante du tampon de texture
* (game->tex[game->tex_num]
* [game->tex_h * game->tex_y + game->tex_x]), 
* Enfin, définit la couleur du
* pixel dans le tampon de jeu(game->buf[y][x])
